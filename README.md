...
1. TypeORM doesn't support optimistic concurrency checks on updates/deletes (e.g. SQL Server timestamp) by default. A workaround is to implement concurrency-checks [through a subscriber](https://github.com/typeorm/typeorm/issues/3608#issuecomment-476352843) or use `createQueryBuilder('entity').update(...).where('entity.timestamp = :timestamp', { timestamp })` instead of `repository.save()`.
1. TypeORM SQLite provider has issues when setting `synchronize = true` in connection config. It creates a new `temporary_TableName` table, copies the data from the original table, drops the original table, and then renames the new table to remove `temporary_` prefix. Unfortunately, the intermediate table drop fails with a `FOREIGN KEY constraint failed` error if there are foreign key references to this table. The issue has been reported [here](https://github.com/typeorm/typeorm/issues/3863).
1. Next.js doesn't correctly set the source files for the source maps of React components (e.g. pages and components). Instead of specifying the relative path, it is prepended instead with `webpack:///`. To fix this behavior, let visual studio know where to look for the source files instead by adding the following configuration in `.vscode/launch.json`:
```
